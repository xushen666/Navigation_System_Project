...
class App {
  ...
  _bindFloatButtons() {
    const panel = document.querySelector('.float-controls');
    // The float buttons are HTML elements atop the canvas. We use them directly.
    // The canvas only draws the decorative background panel.
    document.getElementById('floatZoomIn').addEventListener('click', (e) => { e.stopPropagation(); this.scale *= 1.25; this.scale = clamp(this.scale, 0.01, 40); this._repaint(); });
    document.getElementById('floatZoomOut').addEventListener('click', (e) => { e.stopPropagation(); this.scale *= 0.8; this.scale = clamp(this.scale, 0.01, 40); this._repaint(); });
    document.getElementById('floatReset').addEventListener('click', (e) => { e.stopPropagation(); this._resetView(); });
    document.getElementById('floatClear').addEventListener('click', (e) => { e.stopPropagation(); this._clearHighlights(); });
  }
